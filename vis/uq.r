...
data$sharing_strategy = factor(data$sharing_strategy, levels=c(
  "SRN",
  "Active",
  "Passive",
  "Baseline"
...
sample = sample %>% distinct(country, scenario_adopt, strategy_short, input_cost, financial_cost)
...
financial_cost_10gb = ggplot(sample,
  ...
  labs(
    title="(A) Financial NPV of Universal Broadband by Technology (10 GB/Month Per User)",
    colour=NULL,
...
financial_cost_30gb = ggplot(sample,
  ...
  theme(legend.position = 'bottom') +
  labs(
    title="(B) Financial NPV of Universal Broadband by Technology (30 GB/Month Per User)",
    colour=NULL,
    subtitle = "Interval bars reflect estimates for low and high cost scenarios.",
    x = NULL, y = "Financial Cost (Billions $USD)") +
  scale_y_continuous(expand = c(0, 0), limits = c(0, max_value)) +
...
sample <- data[(data$scenario_capacity == '10 GB/Month'),]
sample <- sample[(sample$sharing_strategy == 'Baseline'),]
sample = sample %>% distinct(country, scenario_adopt, strategy_short, input_cost, government_cost)
...
government_cost_10gb = ggplot(sample,
   aes(x=strategy_short, y=baseline, group=scenario_adopt, fill=scenario_adopt)) +
  geom_bar(stat="identity", position=position_dodge()) +
  geom_errorbar(data=sample, aes(y = baseline, ymin = low, ymax = high),
                position = position_dodge(width = .9), lwd = 0.5,
...
sample = sample %>% distinct(country, scenario_adopt, sharing_strategy, input_cost, financial_cost)

sample <- select(sample, country, scenario_adopt, sharing_strategy, input_cost, financial_cost)
...
financial_cost_10gb = ggplot(sample,
                    aes(x=sharing_strategy, y=baseline, group=scenario_adopt, fill=scenario_adopt)) +
  geom_bar(stat="identity", position=position_dodge()) +
  geom_errorbar(data=sample, aes(y = baseline, ymin = low, ymax = high),
                position = position_dodge(width = .9), lwd = 0.5,
                show.legend = FALSE, width=0.1,  color="#FF0000FF") +
  ...
  theme(legend.position = 'bottom') +
  labs(
    title="(A) Financial NPV using Infrastructure Sharing (10 GB/Month Per User)",
    colour=NULL,
    subtitle = "Interval bars reflect estimates for low and high cost scenarios.",
    x = NULL, y = "Financial Cost (Billions $USD)") +
  scale_y_continuous(expand = c(0, 0), limits = c(0, max_value)) +
...
financial_cost_30gb = ggplot(sample,
  ...
  theme(legend.position = 'bottom') +
  labs(
    title="(B) Financial NPV using Infrastructure Sharing (30 GB/Month Per User)",
    colour=NULL,
    subtitle = "Interval bars reflect estimates for low and high cost scenarios.",
    x = NULL, y = "Financial Cost (Billions $USD)") +
  scale_y_continuous(expand = c(0, 0), limits = c(0, max_value)) +
...
data <- read.csv(file.path(folder_inputs, 'total_cost_estimates.csv'))
...
data <- select(data, scenario_adopt, scenario_capacity, strategy_short, input_cost,
               total_private_cost, total_government_cost,
               total_financial_cost)

data = data %>% distinct(scenario_adopt, scenario_capacity, strategy_short, input_cost,
                         total_private_cost, total_government_cost,
                         total_financial_cost)
...
colnames(data)[colnames(data) == 'total_private_cost'] <- 'Private Cost ($USD)'
colnames(data)[colnames(data) == 'total_government_cost'] <- 'Government Cost ($USD)'
...
data = select(data, scenario_adopt, scenario_capacity, strategy_short, input_cost, Cost_Type, value)

data = spread(data, input_cost, value)

ggplot(data,
         aes(x=strategy_short, y=baseline, group=Cost_Type, fill=Cost_Type)) +
  geom_bar(stat="identity", position=position_dodge()) +
  geom_errorbar(data=data, aes(y = baseline, ymin = low, ymax = high),
                position = position_dodge(width = .9), lwd = 0.5,
                show.legend = FALSE, width=0.1,  color="#FF0000FF") +
  coord_flip() +
