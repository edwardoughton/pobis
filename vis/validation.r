...
# data_2 <- read.csv(file.path(folder, '..', 'results', 'user_costs', 'decile_cost_estimates_2.csv'))
# data_2 = data_2[(data_2$Strategy == '4G(W)' & data_2$Scenario == 'Baseline'),]
# data_2$Capacity = '2 Mbps'
# 
# data = rbind(data_10, data_2)
...
data %>%
  ...
  kable(escape = F, caption = '(A) Cost Results by Decile for the Continent of Africa for ~10 Mbps Per User') %>%
  kable_classic("striped", full_width = F, html_font = "Cambria") %>%
  row_spec(0, align = "c")  %>%
  add_header_above(
    c(" "= 1,"Pop."=1, "Pop."=1,"Area"=1,"Pop.Density"=1, "Cost Type ($Bn)" = 3)) 

    
folder <- dirname(rstudioapi::getSourceEditorContext()$path)
path = file.path(folder, 'figures')
setwd(path)
kableExtra::save_kable(tbl_1, file='validation_10_mbps.png', zoom = 1.5)
...
tbl_2 = 
  ...
  kable(escape = F, caption = '(B) Cost Results by Decile for the Continent of Africa for ~2 Mbps Per User') %>%
  kable_classic("striped", full_width = F, html_font = "Cambria") %>%
  row_spec(0, align = "c")  %>%
  add_header_above(
    c(" "= 1,"Pop."=1, "Pop."=1,"Area"=1,"Pop.Density"=1, "Cost Type ($Bn)" = 3)) 
...
path = file.path(folder, 'figures')
setwd(path)
kableExtra::save_kable(tbl_2, file='validation_2_mbps.png', zoom = 1.5)
